...
Every file under [examples/agents](examples/agents/) is standalone and carries a build tag, so run
one by name rather than building the directory.
...
cd examples
go run agents/01_basic_agent.go
```
...
`CONDUCTOR_AGENT_LLM_MODEL` is optional and defaults to `openai/gpt-4o`. The example prints the
