...
func toolExecutor(t ToolDef) (model.ExecuteTaskFunction, error) {
	...
	return func(task *model.Task) (any, error) {
		...
		in, err := bindInput(t.Name, inType, args)
		if err != nil {
			return nil, err
		}
		...
		blocked, err := checkToolInput(ctx, t, args)
		if err != nil {
			return nil, err
		}
		if blocked != nil {
		...
		if callErr, ok := out[1].Interface().(error); ok && callErr != nil {
			return nil, callErr
		}
		...
		output, err := withStateUpdates(toolOutput(value), state, t.Name)
		if err != nil {
			return nil, err
		}
		return taskResult(task, t.Name, output)
	}, nil
}

// bindInput decodes the model's arguments into a new value of the handler's
// input type. The model sends the wire names the schema advertised; untagged
// fields need them mapped back to Go names before encoding/json binds them.
func bindInput(toolName string, inType reflect.Type, args map[string]any) (reflect.Value, error) {
	...
	if err != nil {
		return in, fmt.Errorf("tool %q: encode input: %w", toolName, err)
	}
	if err := json.Unmarshal(raw, in.Interface()); err != nil {
		return in, fmt.Errorf("tool %q: bind input: %w", toolName, err)
	}
	return in, nil
}

// taskResult builds the completed TaskResult for a tool's output, keeping each
// number as the handler wrote it. The worker's default conversion decodes
// numbers into float64, which turns a json.Number("15000.0") into 15000; the
// server's LLM recorder compares tool results by JSON type, so a tool that
// spells a float the way the Python one does must reach the wire unchanged.
func taskResult(task *model.Task, toolName string, output any) (*model.TaskResult, error) {
	raw, err := json.Marshal(output)
	if err != nil {
		return nil, fmt.Errorf("tool %q: encode output: %w", toolName, err)
	}
	dec := json.NewDecoder(bytes.NewReader(raw))
	dec.UseNumber()
	var data map[string]any
	if err := dec.Decode(&data); err != nil {
		return nil, fmt.Errorf("tool %q: output must be a JSON object: %w", toolName, err)
	}
	res := model.NewTaskResultFromTask(task)
	res.OutputData = data
	res.Status = model.CompletedTask
	return res, nil
}
