...
func TestStateUpdatesPreserveAStructResult(t *testing.T) {
	...
	if got["city"] != "Paris" || got["hits"] != json.Number("2") {
...
func TestStateAbsentFromTheTask(t *testing.T) {
	...
	if !reflect.DeepEqual(out[stateUpdatesKey], map[string]any{"first": json.Number("1")}) {
...
func TestStateUpdatesOnAnUntaggedStruct(t *testing.T) {
	...
	if got["city"] != "Paris" || got["temp_f"] != json.Number("72") {
