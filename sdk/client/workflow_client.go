...
type WorkflowClient interface {
	...
	// Delete deletes a workflow execution from the server using the workflow (execution) ID.
	Delete(ctx context.Context, workflowId string, localVarOptionals *WorkflowResourceApiDeleteOpts) (*http.Response, error)

	// GetExecutionStatus gets a workflow’s execution details using its workflow (execution) ID.
	...
	// GetWorkflowState gets a workflow’s execution details using its workflow (execution) ID.
	...
	// PauseWorkflow pauses an ongoing workflow execution.
	...
	//
	// This method returns immediately with the workflow (execution) ID without waiting for the workflow to be completed.
