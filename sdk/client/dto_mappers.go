...
// toGeneratedTagsFromTagObjects converts []model.TagObject to []orkes.Tag
func toGeneratedTagsFromTagObjects(domainTags []model.TagObject) []orkes.Tag {
	result := make([]orkes.Tag, len(domainTags))
	for i, tag := range domainTags {
		result[i] = toGeneratedTagFromTagObject(tag)
	}
	return result
}
	...
	if len(genIntegration.Tags) > 0 {
		...
		for i, tag := range genIntegration.Tags {
			result.Tags[i] = model.TagObject{
				...
				Type_: GetPointerValue(tag.Type, ""),
			}
		}
	}
	...
	if genApi.Tags != nil {
		...
		for i := range genApi.Tags {
			res.Tags[i] = model.TagObject{
				...
				Type_: GetPointerValue(genApi.Tags[i].Type, ""),
...
func toDomainTags(genTags []orkes.Tag) []model.Tag {
	...
	for i, genTag := range genTags {
		result[i] = model.Tag{
			...
			Type_: GetPointerValue(genTag.Type, ""),
...
// toDomainWorkflowScheduleFromWorkflowSchedule converts orkes.WorkflowSchedule to model.WorkflowSchedule
//
//nolint:gocyclo // Generated mapper function with high complexity
func toDomainWorkflowScheduleFromWorkflowSchedule(genSchedule *orkes.WorkflowSchedule) model.WorkflowSchedule {
	...
	// Map time fields
	if genSchedule.CreateTime != nil {
		result.CreateTime = *genSchedule.CreateTime
	}
	if genSchedule.UpdatedTime != nil {
		result.UpdatedTime = *genSchedule.UpdatedTime
	}
	...
	if genSchedule.Description != nil {
		result.Description = *genSchedule.Description
	}
	...
	if genSchedule.ZoneId != nil {
		result.ZoneId = *genSchedule.ZoneId
	}
	...
	if genSchedule.PausedReason != nil {
		result.PausedReason = *genSchedule.PausedReason
	}
	...
	// Map tags
	if genSchedule.Tags != nil {
		result.Tags = toDomainTags(genSchedule.Tags)
	}
		...
		if tasks[i].TaskDefinition != nil {
			...
			if tasks[i].TaskDefinition.Tags != nil {
				...
				for k, t := range tasks[i].TaskDefinition.Tags {
					tags[k] = map[string]interface{}{"key": t.Key, "type": t.Type_, "value": t.Value}
				}
				td["tags"] = tags
			}
			m["taskDefinition"] = td
