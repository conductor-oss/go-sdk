...
// MetadataResourceApiService
type MetadataResourceApiService struct {
	*APIClient
}

// NewMetadataResourceApiService creates a MetadataResourceApiService
func NewMetadataResourceApiService(apiClient *APIClient) *MetadataResourceApiService {
	if apiClient == nil {
		return nil
	}

	return &MetadataResourceApiService{
		APIClient: apiClient,
	}
}
...
// Create creates workflow definition (backward compat)
func (a *MetadataResourceApiService) Create(ctx context.Context, body model.WorkflowDef, overwrite bool) (*http.Response, error) {
	return a.RegisterWorkflowDef(ctx, overwrite, body)
}
...
// SetTaskTags sets all tags for task definition (replaces existing)
...
// GetWorkflowNamesAndVersions gets all workflow names and versions
func (a *MetadataResourceApiService) GetWorkflowNamesAndVersions(ctx context.Context) (map[string][]int32, *http.Response, error) {
	// Use the generated conductor client method
	req := a.http_conductor.MetadataResourceAPI.GetWorkflowNamesAndVersions(ctx)
...
// ValidateWorkflow validates workflow definition
func (a *MetadataResourceApiService) ValidateWorkflow(ctx context.Context, body model.WorkflowDef) (*http.Response, error) {
	// Convert domain model to generated model
	genWorkflow := toGeneratedWorkflowDefForConductor(body)

	req := a.http_conductor.MetadataResourceAPI.Validate(ctx).WorkflowDef(genWorkflow)
	resp, err := req.Execute()
	if err != nil {
		return resp, wrapGeneratedError(err, resp)
	}
...
// GetAllWorkflows gets all workflow definitions
func (a *MetadataResourceApiService) GetAllWorkflows(ctx context.Context) ([]model.WorkflowDef, *http.Response, error) {
	return a.GetAll(ctx)
}
