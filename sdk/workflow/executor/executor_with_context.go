...
// canonical workflow_input_size_bytes / legacy workflow_input_size gauges.
// A nil input still produces a size observation (0 bytes) so the time-series
// is always present — matches python-sdk which records 0 on empty inputs.
// Serialization failures are swallowed: metrics must never break the
// workflow-start path.
