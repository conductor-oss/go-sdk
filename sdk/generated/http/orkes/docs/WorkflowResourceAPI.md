...
[**GetExecutionStatus**](WorkflowResourceAPI.md#GetExecutionStatus) | **Get** /workflow/{workflowId} | Gets the workflow by workflow (execution) id
[**GetExecutionStatusTaskList**](WorkflowResourceAPI.md#GetExecutionStatusTaskList) | **Get** /workflow/{workflowId}/tasks | Gets the workflow tasks by workflow (execution) id
...
[**GetWorkflowStatusSummary**](WorkflowResourceAPI.md#GetWorkflowStatusSummary) | **Get** /workflow/{workflowId}/status | Gets the workflow by workflow (execution) id
...
[**Search1**](WorkflowResourceAPI.md#Search1) | **Get** /workflow/search | Search for workflow executions based on payload and other parameters
...
[**StartWorkflowAsync**](WorkflowResourceAPI.md#StartWorkflowAsync) | **Post** /workflow/start/{name}/{version} | Start a new workflow asynchronously. Returns the ID of the workflow instance that can be later used for tracking
...
> SignalResponse ExecuteWorkflow(ctx, name, version).StartWorkflowRequest(startWorkflowRequest).RequestId(requestId).WaitUntilTaskRef(waitUntilTaskRef).WaitForSeconds(waitForSeconds).Consistency(consistency).ReturnStrategy(returnStrategy).Execute()
...
func main() {
    ...
    consistency := "consistency_example" // string |  (optional) (default to "DURABLE")
    returnStrategy := "returnStrategy_example" // string |  (optional) (default to "TARGET_WORKFLOW")

    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.WorkflowResourceAPI.ExecuteWorkflow(context.Background(), name, version).StartWorkflowRequest(startWorkflowRequest).RequestId(requestId).WaitUntilTaskRef(waitUntilTaskRef).WaitForSeconds(waitForSeconds).Consistency(consistency).ReturnStrategy(returnStrategy).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `WorkflowResourceAPI.ExecuteWorkflow``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ExecuteWorkflow`: SignalResponse
 ...
 **consistency** | **string** |  | [default to &quot;DURABLE&quot;]
...
> map[string][]Workflow GetWorkflows1(ctx).CorrelationIdsSearchRequest(correlationIdsSearchRequest).IncludeClosed(includeClosed).IncludeTasks(includeTasks).Execute()
...
func main() {
    correlationIdsSearchRequest := *openapiclient.NewCorrelationIdsSearchRequest([]string{"CorrelationIds_example"}, []string{"WorkflowNames_example"}) // CorrelationIdsSearchRequest | 
    includeClosed := true // bool |  (optional) (default to false)
    includeTasks := true // bool |  (optional) (default to false)
    ...
    // response from `GetWorkflows1`: map[string][]Workflow
...
> ScrollableSearchResultWorkflowSummary Search1(ctx).Start(start).Size(size).Sort(sort).FreeText(freeText).Query(query).SkipCache(skipCache).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.WorkflowResourceAPI.Search1(context.Background()).Start(start).Size(size).Sort(sort).FreeText(freeText).Query(query).SkipCache(skipCache).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `WorkflowResourceAPI.Search1``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `Search1`: ScrollableSearchResultWorkflowSummary
    fmt.Fprintf(os.Stdout, "Response from `WorkflowResourceAPI.Search1`: %v\n", resp)
}
```
...
> string StartWorkflowAsync(ctx, name, version).RequestBody(requestBody).CorrelationId(correlationId).Priority(priority).XIdempotencyKey(xIdempotencyKey).XOnConflict(xOnConflict).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.WorkflowResourceAPI.StartWorkflowAsync(context.Background(), name, version).RequestBody(requestBody).CorrelationId(correlationId).Priority(priority).XIdempotencyKey(xIdempotencyKey).XOnConflict(xOnConflict).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `WorkflowResourceAPI.StartWorkflowAsync``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `StartWorkflowAsync`: string
    fmt.Fprintf(os.Stdout, "Response from `WorkflowResourceAPI.StartWorkflowAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiStartWorkflowAsyncRequest struct via the builder pattern
