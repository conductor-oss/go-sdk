...
func TestSkillScriptRunsAsWorker(t *testing.T) {
	...
	// The instructions say to return the tool's line, but models restyle it
	// (a space after the colon, say), so only the payload is checked.
	if !strings.Contains(res.Output, "tool_parity_proof") {
...
func TestSkillAsAgentTool(t *testing.T) {
	...
	if model(t) == mockModel {
		// The parent's second LLM request carries the agent tool's result, and
		// the server puts the sub-workflow's ID in it. That ID is new on every
		// run, so no recording can match; see README, "What has to match".
		t.Skip("agent tool results include a per-run subWorkflowId; the server's recorder cannot replay this")
	}
	dir, marker := writeE2ESkill(t)
