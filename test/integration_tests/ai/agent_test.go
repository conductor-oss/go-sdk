...
// Package integration exercises the agent surface against a Conductor
// server. These tests call a real LLM, so they are behind the `e2e` build tag
// and are not part of `go test ./...`.
//
//	go test -tags integration ./test/integration_tests/ai/ -v
//
// Requirements:
...
//
// Skipping rather than failing on a missing URL keeps `go test -tags integration ./...`
