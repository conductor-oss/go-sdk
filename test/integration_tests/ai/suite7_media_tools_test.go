...
func TestImageOpenai(t *testing.T) {
	...
	agent := &ai.Agent{Name: "e2e_image_openai", Model: model(t),
		...
		Tools:        ai.Tools(tool.Image("gen_image", "openai", "dall-e-3", "Generate an image from a text prompt."))}
...
func TestImageGemini(t *testing.T) {
	...
	agent := &ai.Agent{Name: "e2e_image_gemini", Model: model(t),
		...
		Tools:        ai.Tools(tool.Image("gen_image_gemini", "google_gemini", "imagen-3.0-generate-002", "Generate an image using Gemini Imagen."))}
...
func TestAudioOpenai(t *testing.T) {
	...
	agent := &ai.Agent{Name: "e2e_audio_openai", Model: model(t),
		...
		Tools:        ai.Tools(tool.Audio("gen_audio", "openai", "tts-1", "Convert text to speech audio."))}
