#!/usr/bin/env bash
#
# Run a local Conductor server for the integration tests, with its LLM recorder in
...
#   conductor-server.sh replay <recordings-dir>...  no LLM; responses come from the dirs
#     (several directories are staged into one tree, so one server can play
#     back this repository's recordings and the conductor repository's)
...
case "$mode" in
  record)
    ...
    ;;
  replay)
    [ $# -ge 2 ] || { echo "replay needs at least one recordings directory" >&2; exit 2; }
    if [ $# -eq 2 ]; then
      dir="$2"
    else
      # Several sources: copy each under its own name into one tree the
      # mock provider loads recursively.
      dir="$state/recordings"
      rm -rf "$dir"; mkdir -p "$dir"
      for src in "${@:2}"; do
        mkdir -p "$dir/$(basename "$src")"
        cp -R "$src"/. "$dir/$(basename "$src")/"
      done
    fi
  ...
  plain)
    ...
    ;;
  *)
    echo "usage: $0 record <recordings-dir> | replay <recordings-dir>... | plain | stop" >&2
    exit 2
    ;;
esac
if [[ -n "$dir" ]]; then
