...
**Audience:** Go developers running their first Conductor workflow and worker.

**Prerequisites:** Go 1.23+, and an up & running Conductor server. For details on
how to run Conductor see [our documentation](https://docs.conductor-oss.org). The
examples expect the server to be listening on http://localhost:8080.

**Expected result:** the `greetings` workflow completes and prints
`Output of the workflow:map[Greetings:Hello, Gopher]`.
...
To learn more about workers take a look at [Writing Workers with the Go SDK](workers.md).
...
> [!tip]
> For advanced configuration options and detailed examples see [Connection and authentication](connection-authentication.md).
...
- **No tasks are polled.** Check `CONDUCTOR_SERVER_URL` includes the `/api` suffix.
- **401 or 403 responses.** Orkes requires `CONDUCTOR_AUTH_KEY` and `CONDUCTOR_AUTH_SECRET`; OSS does not.
- **Workflow starts but never completes.** The worker registers for task type `greet`; confirm the task name in the definition matches the name passed to `StartWorker`.

Next: [Workflows](workflows.md), [Workers](workers.md), and
[Connection and authentication](connection-authentication.md).
