...
- Playback: `scripts/run-agents-playback.sh` runs every `examples/agents` example against a
  conductor playback server with the mock model; `.github/workflows/agents-playback.yml` is the
  CI job. Historically there was a per-scenario integration suite; the streaming test
