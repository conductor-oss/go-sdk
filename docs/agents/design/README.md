...
- **Behaviour.** `test/integration_tests/ai/` runs against a live Conductor server, or against
  recorded model traffic in playback, and each test asserts the worker-side effect — a counter
  incremented, a value bound — not just that the run finished. It is behind the `integration`
  build tag; see that directory's README for running and recording.
