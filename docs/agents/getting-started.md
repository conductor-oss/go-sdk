...
Every file under [examples/agents](../../examples/agents/) is standalone and carries a build tag, so
run one by name rather than building the directory.
...
The examples default `Model` to `openai/gpt-4o` when `CONDUCTOR_AGENT_LLM_MODEL` is unset.
